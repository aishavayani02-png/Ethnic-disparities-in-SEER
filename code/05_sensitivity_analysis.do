/****************************************************************************************
Sensitivity analysis: treat unknown/unstaged cancers as distant stage.

Run this file from the repository root after editing code/00_config.do. It begins
with the tidy data, repeats the primary-analysis eligibility restrictions, recodes
unknown/unstaged disease to distant, and saves private working result files.
****************************************************************************************/
...
foreach site of global ANALYSIS_SITES {
	...
	* load dataset
	...
	foreach j of local sexlist {
		frame reset
		cd "$TIDY_DATA_DIR"
		use `site'_tidy.dta, clear
		
		* Restrict to women with breast cancer
		if "`site'" == "Breast" {
			keep if sex == 2
		}
		
		* Drop appendix site in colon
		if "`site'" == "Colon"{
			drop if site == 0
		}
		
		* Keep primary by international rules
		keep if PrimaryByInternationalRules == "Yes"
		drop PrimaryByInternationalRules
		
		* Remove death certificate/autopsy only case
		drop if SurvMonthsFlag == ///
		"Not calculated because a Death Certificate Only or Autopsy Only case"
		
		* Survival times are in complete number of months so add 0.5 to be on average closer
		replace surv_mm = surv_mm + 0.5
		
		* Keep those with 0 days survival but replace with a survival time of 0.5 days
		replace surv_mm = 0.5/30.5 if SurvMonthsFlag == ///
		"Complete dates are available and there are 0 days of survival"
		drop SurvMonthsFlag
		
		* Drop 90+ age category
		drop if age == 90
		
		* Drop if age < 15 (to match ICSS weights)
		drop if age<15
		
		* Drop Other and unknown race
		drop if race == 3 // other
		drop if race == 4 // unknown
		
		* Drop missing stage / in-situ
		drop if stage == 0 // in-situ
		
		* Replace unknown stage with distant
		replace stage = 3 if stage == 4 // unknown/unstaged
